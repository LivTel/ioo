...
; Insert the serias fiber optic transmit command
	...
	MOVE	X0,Y:(R1)+              ; cjm Add  Y:SXMIT to PXL_TBL
        ; cjm Add FIRST_CLOCKS waveforms to PXL_TBL
...
; Generate the binning waveforms if needed
	MOVE	Y:<NSBIN,A
	SUB	#1,A
	JLE	<GEN_RST        ; cjm if Y:<NSBIN == 1 go straight to GEN_RST
	DO	A1,L_BIN        ; cjm loop over Y:<NSBIN-1
        ;  cjm Add CLOCK_LINE to PXL_TBL
...
; cjm add RESET_VIDEO to PXL_TBL
...
; cjm add CHARGE_DUMP to PXL_TBL
