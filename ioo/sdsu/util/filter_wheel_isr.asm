; filter_wheel_isr.asm
       COMMENT * 
	$Header: /space/home/eng/cjm/cvs/ioo/sdsu/util/filter_wheel_isr.asm,v 1.3 2011-07-26 14:00:21 cjm Exp $
...
; FW_TIMEOUT_INDEX = (FW_TIMEOUT_INDEX+1)
	...
	ADD	X0,A					; A = FW_TIMEOUT_INDEX+1
	MOVE	A,Y:FW_TIMEOUT_INDEX			; FW_TIMEOUT_INDEX = A (FW_TIMEOUT_INDEX+1)
; de bounce, dont't check proximity for 100 loops (0.1s)
	MOVE	#>100,X0
...
; check proximity sensor pattern 
	...
	TST	A
; If no proximity bits are set we have moved out of a position
	JEQ	<FW_ISR_MOVED_OUT
...
; check proximity sensor pattern 
	...
	AND	X1,A				; A = A & X1 (#DIG_IN_PROXIMITY_MASK), isolate DIG_IN proximity bits
	TST	A
; If proximity bits are set we have moved in to a position
	JNE	<FW_ISR_MOVED_IN
; if FW_TIMEOUT_INDEX >= Y:<FW_TIMEOUT_COUNT move timeout
	MOVE	Y:FW_TIMEOUT_INDEX,A			; A = FW_TIMEOUT_INDEX
	MOVE	Y:FW_TIMEOUT_COUNT,X0
