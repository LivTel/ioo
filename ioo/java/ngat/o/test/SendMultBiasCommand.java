// SendMultBiasCommand.java
// $Header: /space/home/eng/cjm/cvs/ioo/java/ngat/o/test/SendMultBiasCommand.java,v 1.1 2011-11-23 10:59:38 cjm Exp $
package ngat.o.test;
...
/**
 * This class sends a MULTBIAS command to O. 
 * @author Chris Mottram
 * @version $Revision: 1.1 $
 */
public class SendMultBiasCommand
{
	/**
	...
	/**
	 * Number of exposures for the MULTBIAS to take. 
	 * Defaults to zero, which should cause MULTBIAS to return an error.
	 */
	...
	/**
	 * This routine creates a MULTBIAS command. 
	 * @return An instance of MULTBIAS.
	 * @see #exposureCount
	 */
	private MULTBIAS createMultBias()
	{
		String string = null;
		MULTBIAS multBiasCommand = null;

		multBiasCommand = new MULTBIAS("SendMultBiasCommand");
		multBiasCommand.setNumberExposures(exposureCount);
		return multBiasCommand;
	}

	/**
	 * This is the run routine. It creates a MULTBIAS object and sends it to the using a 
	...
	private boolean run() throws Exception
	{
		...
		issCommand = (ISS_TO_INST)(createMultBias());
	...
	private boolean getThreadResult(SicfTCPClientConnectionThread thread)
	{
		...
		else
		{
			if(thread.getDone().getSuccessful())
			{
				...
				if(thread.getDone() instanceof MULTBIAS_DONE)
				{
					System.out.println("\tFilename:"+
						((MULTBIAS_DONE)(thread.getDone())).getFilename());
				}
				retval = true;
			}
			else
	...
	/**
	 * The main routine, called when SendMultBiasCommand is executed. This initialises the object, parses
	...
	{
		boolean retval;
		SendMultBiasCommand smbc = new SendMultBiasCommand();

		smbc.parseArgs(args);
		smbc.init();
		if(smbc.address == null)
		{
