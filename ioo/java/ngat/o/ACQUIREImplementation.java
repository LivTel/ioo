...
// $Header: /space/home/eng/cjm/cvs/ioo/java/ngat/o/ACQUIREImplementation.java,v 1.5 2014-04-08 08:50:31 eng Exp $
package ngat.o;
...
public class ACQUIREImplementation extends FITSImplementation implements JMSCommandImplementation
{
     ...
     */
    public final static String RCSID = new String("$Id: ACQUIREImplementation.java,v 1.5 2014-04-08 08:50:31 eng Exp $");
    /**
	...
	try
		...
		try
			...
			o.log(Logging.VERBOSITY_VERBOSE,
			      ":computeRADecOffset: Target original position: at T=0.0s: "+acquireRAString+", "+acquireDecString);
			...
			if (moving) {
			    ...
			    checkRARads =  acquireRARads + raRateRadPerSec*((double)timeStartExposure + (double)exposureLength/2.0 - (double)rateTime)/1000.0;
			    checkDecRads = acquireDecRads + decRateRadPerSec*((double)timeStartExposure + (double)exposureLength/2.0 - (double)rateTime)/1000.0;
			...
			coordinate = WCSTools.wcsToPixel(handle,
							 checkRARads*180.0/Math.PI,
	...
	if (moving) 
	    offsetRARads = acquireRARads + raRateRadPerSec*((double)timeStartExposure + (double)exposureLength/2.0 - (double)rateTime)/1000.0 - raRads;
	else
	...
	if (moving)
	    offsetDecRads = acquireDecRads + decRateRadPerSec*((double)timeStartExposure + (double)exposureLength/2.0 - (double)rateTime)/1000.0 - decRads;
	else
